...
class SpamDetection:
    ...
    def personalizedPageRank(self,R,M,S,steps=40,tolerance=0.002):
        alpha=0.85
        nodes=len(R)
    ...
    def prediction(self,newUsers,ts):
        ...
        f = open(filenm, 'w')
        for key,val in indexNodes:
            s={"reviewerId":key, "rank": R[val]}
            f.write(s+"\n")
        f.close()
    ...
    def apply_nmf(self, ts):
        ...
        for i in range(0,len(nodes)):
            x = {"reviewerID": nodes[i], "burstiness" :self.float_precision(U[i][0]), "rate_deviation":self.float_precision(U[i][1]),"content_similarity":self.float_precision(U[i][2]),"first_time":self.float_precision(U[i][3])}
            f.write(str(x)+"\n")
        f.close()
        ...
        for i in range(0,len(nodes)):
            ...
            x = {"reviewerID": uid, "spam":self.float_precision(nmf_mdl.W[i][0]) ,"poss_spam":self.float_precision(nmf_mdl.W[i][1]) ,"not_spam":self.float_precision(nmf_mdl.W[i][2]),"spam_score_ts":self.float_precision(spamicity_score), "spam_score_total":self.float_precision(spamicity_score_total)}
            f.write(str(x)+"\n")
        f.close()
        self.spamicity_score(ts)
    ...
    #Timestamp count
        ...
        while end_date <= final_end_date:
            ...
            self.apply_nmf(cnt)
            t1=time();
            print "NMF Done %f in : %f" % (t1 - t0), "secs.\n";
            #if cnt ==10:
            break
